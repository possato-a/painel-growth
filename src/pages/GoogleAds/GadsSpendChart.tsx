      ...
      {payload.map((entry) => (
        <div key={entry.name} className="flex items-center gap-2 mb-1">
          ...
          <span className="text-notion-text-primary text-xs font-medium">
            {entry.name === 'Investimento' ? fmtCurrency(entry.value) : entry.value.toFixed(1)}
          </span>
        </div>
      ))}
    </div>
...
export function GadsSpendChart({ data, isLoading }: Props) {
  ...
  return (
    <div className="bg-notion-bg-primary rounded shadow-notion-md p-5">
      <div className="flex items-center justify-between mb-5">
        <h2 className="text-base font-semibold text-notion-text-primary">Desempenho Diário</h2>
        {data && data.length > 0 && (
          <span className="text-xs text-notion-text-tertiary">{data.length} dias</span>
        )}
      </div>
      ...
      {isLoading ? (
        ...
        <ResponsiveContainer width="100%" height={240}>
          <AreaChart data={chartData} margin={{ top: 4, right: 8, left: 0, bottom: 0 }}>
            <defs>
              <linearGradient id="gadsColorSpend" x1="0" y1="0" x2="0" y2="1">
                <stop offset="5%"  stopColor="#2383E2" stopOpacity={0.15} />
                <stop offset="95%" stopColor="#2383E2" stopOpacity={0} />
              </linearGradient>
              <linearGradient id="gadsColorConv" x1="0" y1="0" x2="0" y2="1">
                <stop offset="5%"  stopColor="#0F7B6C" stopOpacity={0.20} />
            ...
            <Area
              ...
              stroke="#0F7B6C"
              strokeWidth={2}
              fill="url(#gadsColorConv)"
              dot={false}
              activeDot={{ r: 4, strokeWidth: 0, fill: '#0F7B6C' }}
            />
          </AreaChart>
        </ResponsiveContainer>
      )}
